...
class LineLabel(so.Mark):
    ...
    def _compute_target_positions(
        ...
        *,
        ...
        for ax, rows in data.items():
            ...
            offsets = np.array([[_resolve_fontsize(row["_keys"]) * (5 / 3) + offset] for row in rows])
            ...
            b = sorted_points[other].squeeze(1) - (min_point + np.arange(num_points) * sorted_offsets.squeeze(1))
            ...
            logging.info("Found line label positions with final objective value: %f", objective_value)
    ...
    def _plot(self, split_gen, scales, orient):
        data_by_axes: dict[mpl.axes.Axes, list[Mapping[str, Any]]] = collections.defaultdict(list)  # pyright: ignore
